...
GRANT ALL PRIVILEGES ON SCHEMA school TO admin;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA school TO admin;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA school TO admin;

-- Grant permissions to teacher
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA school TO teacher;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA school TO teacher;

-- Grant permissions to student
GRANT SELECT ON school."Class" TO student;
GRANT SELECT ON school."Homework" TO student;
GRANT SELECT ON school."Attendance" TO student;
GRANT SELECT ON school."Fee" TO student;

-- Set default privileges for future tables
ALTER DEFAULT PRIVILEGES IN SCHEMA school GRANT SELECT, INSERT, UPDATE ON TABLES TO teacher;
ALTER DEFAULT PRIVILEGES IN SCHEMA school GRANT SELECT ON TABLES TO student;

-- Example of how to restrict student access to their own data
-- Assuming there is a student_id column in the Homework table
CREATE OR REPLACE FUNCTION restrict_student_access()
RETURNS TRIGGER AS $$
BEGIN
    IF (SELECT current_user) = 'student' THEN
        IF NEW."studentId" IS DISTINCT FROM current_user THEN
            RAISE EXCEPTION 'Students can only see their own data.';
        END IF;
    END IF;
    RETURN NEW;
END;
...
-- Create a trigger for the Homework table to enforce the restriction
CREATE TRIGGER restrict_student_access_trigger
BEFORE INSERT OR UPDATE ON school."Homework"
FOR EACH ROW EXECUTE FUNCTION restrict_student_access();
